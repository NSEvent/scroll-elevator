...
enum JumpDispatcher {
    ...
    private static let keyPageUp: CGKeyCode = 116
    private static let keyPageDown: CGKeyCode = 121
    ...
    static func jump(_ direction: JumpDirection, target: ScrollTarget, rule: JumpRule) {
        ...
        case .homeEnd:
            post(chord: KeyChord(keyCode: direction == .top ? keyHome : keyEnd, flags: []), to: target)
        case .cmdHomeEnd:
            post(chord: KeyChord(keyCode: direction == .top ? keyHome : keyEnd, flags: .maskCommand), to: target)
        }
    }

    /// One page up/down (long-press action). Keeps it key-based — page-stepping
    /// is inherently a focused-app interaction.
    static func page(_ direction: JumpDirection, target: ScrollTarget) {
        guard promptForAccessibilityIfNeeded() else { return }
        post(chord: KeyChord(keyCode: direction == .top ? keyPageUp : keyPageDown, flags: []), to: target)
    }
    ...
    private static func defaultChord(for bundleIdentifier: String?, direction: JumpDirection) -> KeyChord {
        if let bundleIdentifier {
            if homeEndApps.contains(bundleIdentifier) {
                return KeyChord(keyCode: direction == .top ? keyHome : keyEnd, flags: [])
            }
            if cmdHomeEndApps.contains(bundleIdentifier) {
                return KeyChord(keyCode: direction == .top ? keyHome : keyEnd, flags: .maskCommand)
            }
        }
